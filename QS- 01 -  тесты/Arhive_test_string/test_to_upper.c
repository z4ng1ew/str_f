...
START_TEST(hello_world_diy_func) {
  ...
  char* str_s21_to_upper = s21_to_upper(str_origin);
  ck_assert_str_eq(str_s21_to_upper, str_to_upper_c);
  free(str_s21_to_upper);
}
END_TEST
...
START_TEST(hello_world_str) {
  ...
  char* str_s21_to_upper = s21_to_upper(str_origin);
  ck_assert_str_eq(str_s21_to_upper, str_check);
  free(str_s21_to_upper);
}
END_TEST
