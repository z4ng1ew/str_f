...
START_TEST(num) {
  ...
  ck_assert_ptr_eq(memchr(str, '6', 6), s21_memchr(str, '6', 6));
}
END_TEST

START_TEST(num_chars_string) {
  const char str[] = "123456789abcdefghijklmnopqrstuvwxyz";
  ck_assert_ptr_eq(memchr(str, 'a', 20), s21_memchr(str, 'a', 20));
}
END_TEST

START_TEST(end_symb) {
  const char str[] = "\0";
  ck_assert_ptr_eq(memchr(str, '\0', 2), s21_memchr(str, '\0', 2));
}
END_TEST

START_TEST(spaces) {
  const char str[] = "        ";
  ck_assert_ptr_eq(memchr(str, ' ', 8), s21_memchr(str, ' ', 8));
}
END_TEST
...
START_TEST(null_test) {
  ...
  ck_assert_ptr_eq(memchr(str, 'F', 5), s21_memchr(str, 'F', 5));
}
END_TEST

START_TEST(not_enough_bytes) {
  const char str[] = "ABCDEF";
  ck_assert_ptr_eq(memchr(str, 'F', 3), s21_memchr(str, 'F', 3));
}
END_TEST

START_TEST(find_num_in_array) {
  int array[] = {1, 2, 3, 777, 5, 99};
  ck_assert_ptr_eq(s21_memchr(array, 777, sizeof(int) * 6),
