...
Функция strrchr расшифровывается как "string reverse character". Название функции указывает на её предназначение: поиск последнего вхождения указанного символа в строке.

str: сокращение от "string" (строка).
r: сокращение от "reverse" (обратный), что указывает на поиск с конца строки.
chr: сокращение от "character" (символ).


Описание функции:

Функция strrchr ищет последнее вхождение заданного символа в строке и возвращает указатель на это вхождение. Если символ не найден, возвращается NULL.
*/
...
char *s21_strrchr(const char *str, int c) {
    ...
    if (str != s21_NULL) { //проверка на случай если кокой-нибудь идиот попытается ввести в str1 - NULL
        ...
        int len = s21_strlen(str); // В части кода strlen(str) происходит вызов функции strlen для определения длины строки str. Функция strlen принимает указатель на строку типа const char * и возвращает количество символов в строке, не включая завершающий нулевой символ (\0) в виде типа данных size_t.
        ...
        for (int i = len; i >= 0; i--) { //После каждой итерации значение i уменьшается на 1. Это обеспечивает движение по строке в обратном порядке, от последнего символа к первому.
            ...
            if (str[i] == c) { //если нашлось совпадение
                ...
                */
                ...
                s21_strlen(string) вернет 14 (длина строки без завершающего нулевого символа).
...
//Новая версия от 14:54 30 июня 2024