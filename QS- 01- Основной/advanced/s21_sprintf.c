...
int s21_sprintf(char *str, const char *format, ...) {
  ...
  char *counter = str;  // счетчик для возврата функции спритф
  while (*format != '\0') {
    flags full_flags = {0};
    full_flags.tochnost = -1;
    // Инициализация структуры флагов
    if (*format != '%') {
      *str++ = *format++;
    } else {  // Парсинг значений структуры flags
      format = get_full_flags(format, &full_flags, args);
      // Запись форматированных строк в str
  ...
  *str = '\0';
  ...
  va_end(args);
  return str - counter;  // вернем значение количества символов в строке
}
...
// Считываем из форматной строки [Флаги]
...
// Считываем из форматной строки [Ширина]
...
// Считываем из форматной строки [Точность]
...
// Считываем из форматной строки [Длину]
...
// После парсинга всех элементов начинаем в отдельных функциях записывать
// значения в buffer, а потом в str в s21_sprintf
...
// функция трансформирующая спецификатор 'c'
...
// функция трансформирующая спецификатор 'di'
...
// функция трансформирующая спецификатор 's'
...
// функция трансформирующая спецификатор 'oxXu'
...
// функция трансформирующая спецификатор 'gGeEf'
...
// возвращает степень k
int scientific_notaion(long double *num) {
  int k = 0;
  if (fabsl(*num) > 1) {  // случай 2.134..
    ...
    k *= -1;  // так как степень отриц.
  }
  return k;
}

// Добавляем нуль если степень это одна цифра
...
// функция трансформирующая спецификатор 'p'
void ptr_to_str(unsigned long int *ptr, char str[]) {
  ...
  if (ptr == s21_NULL) {
    ...
    // продолжаем добавлять символы в результирующую строку str
    str[count++] = '0';
    str[count++] = 'x';
    ...
    if (decimal_number == 0) {
      ...
      // указатель занимает 8 байт == 16 шестнадцатеричным символам.
...
// Функция для форматирования[.точность]
...
// Функция для форматирования[Флаг]
...
// Функция для форматирования [Ширина]
...
// Функция для форматирования знаковые [Длина]
...
// Функция для форматирования беззнаковые [Длина]
...
// Конвертации строки в число, идем по строке пока не встретим что-то отличное
// от числа, а все что числа записываем в переменную и выводим в конце, если
// числа нет то вывести 0
...
// Проверяем на число. Если да, то вовзращает 1, если нет 0
